...
class ImageNetDownloader:
    ...
    def getImageURLsOfWnid(self, wnid):
        ...
        contents_bt = f.read()
        contents_text = contents_bt.decode(encoding='utf-8', errors='ignore')
    ...
    def downloadOriginalImages(self, wnid, username, accesskey):
        download_url = 'http://www.image-net.org/download/synset?wnid=%s&username=%s&accesskey=%s&release=latest&src=stanford' % (
        wnid, username, accesskey)
        try:
